...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf($_POST['csrf_token'] ?? '')) { $err='Token tidak valid'; }
        ...
        if (isset($_POST['action']) && $_POST['action']==='create') {
            ...
            if ($name) {
                try { $pdo->prepare('INSERT INTO destinations (name) VALUES (?)')->execute([$name]); $msg='Tujuan ditambah'; } catch (Throwable $e) { $err='Gagal'; }
            } else { $err='Isi nama'; }
        } elseif (isset($_POST['action']) && $_POST['action']==='update' && $user['role']==='superadmin') {
            $id=(int)($_POST['id']??0); $name=trim($_POST['name']??'');
            if ($id && $name) { try { $pdo->prepare('UPDATE destinations SET name=? WHERE id=?')->execute([$name,$id]); $msg='Diupdate'; } catch (Throwable $e) { $err='Gagal'; } } else { $err='Data tidak lengkap'; }
        }
    }
}
$rows = $pdo->query('SELECT * FROM destinations ORDER BY name')->fetchAll();
?>
<?php include __DIR__ . '/../layout/header.php'; ?>
<div class="row">
  <div class="col-md-6">
    <h4><i class="fa-solid fa-location-dot me-2"></i>Tujuan Kirim</h4>
